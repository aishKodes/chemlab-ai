...
ON DUPLICATE KEY UPDATE title = VALUES(title), book_id = VALUES(book_id), subject_id = VALUES(subject_id), status = IF(status = 'published', status, VALUES(status)), updated_at = NOW();
...
INSERT INTO chapters (book_id, class_id, subject_id, chapter_number, title, slug, status, created_at, updated_at)
SELECT b.id, c.id, s.id, chapter_number, title, slug, 'draft', NOW(), NOW()
FROM classes c
JOIN subjects s ON s.class_id = c.id
...
SELECT ch.id, ch.class_id, ch.subject_id, topic_title, topic_slug, topic_order, 'beginner', 'draft', NOW(), NOW()
FROM chapters ch
...
ON DUPLICATE KEY UPDATE title = VALUES(title), order_index = VALUES(order_index), difficulty = VALUES(difficulty), status = IF(status = 'published', status, VALUES(status)), updated_at = NOW();
