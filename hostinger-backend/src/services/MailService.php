...
final class MailService
{
    ...
    public function sendTemplate(string $to, string $templateKey, array $data, ?int $userId = null): array
    {
        ...
        if (!$this->isConfigured() || !class_exists(PHPMailer::class)) {
            $this->writeMailSnapshot($to, $subject, $html);
            $this->updateLog($logId, 'failed', 'SMTP or PHPMailer is not configured.');
            return ['sent' => false, 'status' => 'failed', 'error' => 'SMTP or PHPMailer is not configured.'];
        }
